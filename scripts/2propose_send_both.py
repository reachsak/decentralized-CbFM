...
def propose(store_value,amount,tokens):
    account = get_account()
    args1 = (store_value,amount)
    args2 = (store_value,tokens)
    eth_encoded_function =Contract.from_abi("TimeLock", TimeLock[-1], TimeLock.abi).sendEther.encode_input(
        *args1
    )
    
    token_encoded_function = Contract.from_abi("GovernanceToken", GovernanceToken[-1], GovernanceToken.abi).sendTokens.encode_input(
        *args2
    )
    propose_tx = MoralisGovernor[-1].propose(
        [TimeLock[-1].address,GovernanceToken[-1].address],
        [0],
        [eth_encoded_function,token_encoded_function],
...
def main():
    ...
    tokens=22222
    proposal_id=propose(STORE_VALUE,amount,tokens)
    print(proposal_id)
    ##original
